...
AShooterCharacter::AShooterCharacter() :
	...
	bDead(false),
	// Health Status
	HealthState(EHealthState::EHS_Normal),
	MiddleHealth(40.f),
	MinHealth(15.f)
{
...
float AShooterCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (Health - DamageAmount <= 0.f)
	{
		...
		if (EnemyController)
		{
			EnemyController->GetBlackboardComponent()->SetValueAsBool(FName("CharacterDeath"), true);
		}
	}
	else
	{
...
void AShooterCharacter::SetHealthStatus()
{
	switch (HealthState)
	{
	case EHealthState::EHS_Normal:
		if (Health < MiddleHealth)
		{
			HealthState = EHealthState::EHS_BelowMinimum;
		}
		break;

	case EHealthState::EHS_BelowMinimum:
		if (Health < MinHealth)
		{
			HealthState = EHealthState::EHS_Exhausted;
		}
	...
	case EHealthState::EHS_Exhausted:
		if (Health >= MinHealth)
		{
			HealthState = EHealthState::EHS_BelowMinimum;
		}
...
void AShooterCharacter::CameraInterpZoom(float DeltaTime)
{
	...
	if (bAiming)
	{
		// 현재 값에서 대상까지 보간하여 카메라의 현재 시야 값을 얻음 (bAiming이 true일때 확대된 값)
...
void AShooterCharacter::TraceForItems()
{
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitWeapon)
			{
				if (HighlightedSlot == -1)			// Not Being Highlighted
				{
			...
			{
				if (HighlightedSlot != -1)			// Being Highlighted
				{
...
void AShooterCharacter::SendBullet()
{
	...
	if (BarrelSocket)
	{
		...
		// GetBeamEndLocation : 무언가를 명중할 때 라인 트레이스의 충격 위치를 제공
...
void AShooterCharacter::CameraShake()
{
	if (MatineeCameraShake)
	{
		GetWorld()->GetFirstPlayerController()->PlayerCameraManager->StartCameraShake(MatineeCameraShake, 1.0f);
	}
}
...
{
	// 빠르게 교환할 수 있도록 ECS_Equipping도 추가
